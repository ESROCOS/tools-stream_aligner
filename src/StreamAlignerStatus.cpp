...
std::ostream &operator<<(std::ostream &os, const stream_aligner::StreamAlignerStatus &status)
{
    using ::operator <<;
    os
	<< "time agg: \t"
	...
	if(it->active)
        {
            os << cnt << "\t";
            timers(os, *it, status.current_time); 
        }
	cnt++;
    }
    return os;
...
std::ostream& timers(std::ostream& os, const stream_aligner::StreamStatus& status, base::Time current_time)
{
    using ::operator <<;
    os 	
